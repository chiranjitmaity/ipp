...
export const BlogService = {
    getAllBlogs: async (): Promise<BlogPost[]> => {
        try {
            ...
            // 3. Merge them
            const mergedBlogs = [...dynamicBlogs, ...staticBlogs];

            // 4. Generate missing blogs for tools
            const existingSlugs = new Set(mergedBlogs.map(b => b.slug));
            const generatedBlogs: BlogPost[] = [];

            TOOLS.forEach(tool => {
                const potentialSlug = `how-to-use-${tool.id}`;
                // Check if a blog already exists for this tool (either by matching toolId or the standard slug)
                const alreadyExists = mergedBlogs.some(b => b.toolId === tool.id || b.slug === potentialSlug);
            ...
            return [...mergedBlogs, ...generatedBlogs];
        } catch (error) {
