...
export const BlogService = {
    getAllBlogs: async (): Promise<BlogPost[]> => {
        let allBlogs: BlogPost[] = [...BLOG_POSTS];

        try {
            // 1. Try to get dynamic blogs from MongoDB
            ...
            // Merge dynamic blogs into the list
            allBlogs = [...dynamicBlogs, ...allBlogs];

        } catch (error) {
            console.error("Failed to fetch dynamic blogs (using static only):", error);
            // Continue with just static blogs
        }

        // 2. Auto-generate missing blogs for tools (Robuster implementation)
        const generatedBlogs: BlogPost[] = [];

        TOOLS.forEach(tool => {
            const potentialSlug = `how-to-use-${tool.id}`;
            // Check if a blog already exists for this tool
            const alreadyExists = allBlogs.some(b => b.toolId === tool.id || b.slug === potentialSlug);
        ...
        return [...allBlogs, ...generatedBlogs];
    },
    ...
    getBlogBySlug: async (slug: string): Promise<BlogPost | undefined> => {
        ...
        try {
            ...
            console.error("Error fetching blog by slug (checking static/dynamic):", error);
            // Fallthrough to static/dynamic generation
        }
